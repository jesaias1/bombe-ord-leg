-- Update start_new_game to be host-only with user_id parameter
...
BEGIN
  ...
  -- pick first alive player (join order)
  ...
  IF v_first_player IS NULL THEN
    -- if empty room, just OK so UI doesn't break
    RETURN json_build_object('success', true, 'empty', true);
  END IF;

  SELECT s.syllable INTO v_next_syllable
  FROM syllables s JOIN rooms r ON r.id=p_room_id
  WHERE s.difficulty = r.difficulty AND s.word_count >= 5
  ...
  INSERT INTO games(
    ...
    timer_duration, timer_end_time, used_words, correct_words, incorrect_words, round_number
  ) VALUES(
  ...
  RETURN json_build_object('success', true, 'next_player', v_first_player::text, 'next_syllable', v_next_syllable);
END;
$$;