...
DROP FUNCTION IF EXISTS public.is_room_creator(text);

-- 3) Helper: is current user the room creator? (no recursion; reads rooms only)
...
-- 4) Helper: is current user a member of the room?
...
-- 5) Ensure helpers are owned by a superuser/owner so they bypass RLS
