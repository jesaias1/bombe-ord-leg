-- 1A) Add nullable winner reference
ALTER TABLE IF EXISTS public.games
  ADD COLUMN IF NOT EXISTS winner_player_id UUID NULL;
...
BEGIN
  IF NOT EXISTS (
    ...
    AND constraint_name = 'games_winner_player_id_fkey'
  ) THEN
...
-- 1B) Update handle_timeout to set winner when game ends
...
-- 2) Make join idempotent without reviving mid-game
...
BEGIN
  ...
  -- If player already exists, just update their name but keep lives/is_alive as-is
  IF v_existing_player_id IS NOT NULL THEN
    UPDATE players 
    SET name = COALESCE(NULLIF(TRIM(p_name), ''), 'Spiller')
    WHERE id = v_existing_player_id;
    
    RAISE NOTICE 'Updated existing player % name only (keeping lives/is_alive)', v_existing_player_id;
    RETURN v_existing_player_id;
  END IF;
  
  -- Create new player with 3 lives
  INSERT INTO players (room_id, user_id, name, lives, is_alive)
  VALUES (p_room_id, p_user_id, COALESCE(NULLIF(TRIM(p_name), ''), 'Spiller'), 3, true)
