-- Fix game rule: Invalid words should NOT decrement HP or advance turn
-- Only timeouts should decrement HP and advance the turn
...
BEGIN
  ...
  -- Word is valid - advance turn as before
  ...
  SET 
    ...
    used_words = CASE 
      WHEN trim(lower(p_word)) = ANY(COALESCE(g.used_words, ARRAY[]::text[])) 
      THEN g.used_words 
      ELSE array_append(COALESCE(g.used_words, ARRAY[]::text[]), trim(lower(p_word)))
    END,
    correct_words = CASE 
      WHEN trim(lower(p_word)) = ANY(COALESCE(g.correct_words, ARRAY[]::text[])) 
      THEN g.correct_words 
      ELSE array_append(COALESCE(g.correct_words, ARRAY[]::text[]), trim(lower(p_word)))
    END,
...
-- Update handle_timeout to use consistent response format
