-- Drop overloaded function signatures to avoid confusion
DROP FUNCTION IF EXISTS public.submit_word();
DROP FUNCTION IF EXISTS public.submit_word(text, text, text);
DROP FUNCTION IF EXISTS public.handle_timeout(text, text);

-- Create clean UUID-based submit_word function
CREATE OR REPLACE FUNCTION public.submit_word(p_room_id uuid, p_user_id uuid, p_word text)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  FROM players p 
  WHERE p.room_id = p_room_id::text AND p.user_id = p_user_id::text
  FOR UPDATE;
...
-- Create clean UUID-based handle_timeout function
CREATE OR REPLACE FUNCTION public.handle_timeout(p_room_id uuid, p_user_id uuid)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
