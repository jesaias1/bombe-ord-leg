-- Drop existing submit_word functions to avoid PostgREST ambiguity
DROP FUNCTION IF EXISTS public.submit_word(text, uuid, text);

-- Create atomic submit_word with turn_seq protection
...
BEGIN
  ...
  IF v_next_syllable IS NULL THEN
    RETURN json_build_object('success', false, 'error', 'No syllable found');
  END IF;
